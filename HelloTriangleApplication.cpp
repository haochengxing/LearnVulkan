...
struct EVertex {
	...
	bool operator==(const EVertex& other) const
	{
...
namespace std {
	template<> struct hash<EVertex> {
		size_t operator()(EVertex const& vertex) const
		{
...
private:
   ...
   //交换链图像由交换链自己负责创建，并在交换链清除时自动被清除，不需要我们自己进行创建和清除操作
	...
	{
		m_TextureImageView = __createImageView(m_TextureImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT);
	}
	...
	void __loadModel() {
		...
		std::unordered_map<EVertex, uint32_t> uniqueVertices{};
